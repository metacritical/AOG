...
* Configuration
  ...
  : (require 'org-page)
  : (setq aog/repository-directory "path/to/your/org/repository")   ;; the repository location
  : (setq aog/site-domain "http://your.personal.site.com/")         ;; your domain
  : ;;; the configuration below you should choose one, not both
  : (setq aog/personal-disqus-shortname "your_disqus_shortname")    ;; your disqus commenting system
  : (setq aog/personal-duoshuo-shortname "your_duoshuo_shortname")  ;; your duoshuo commenting system
  : (setq aog/hashover-comments t)                                   ;; activate hashover self-hosted comment system
...
* Publication
  ...
  : (aog/do-publication t nil nil t)
...
** Site Preview
  ...
  : (aog/do-publication-and-preview-site)

  After executing this command, your site will be published to the =aog/site-preview-directory= which defaults to
  =~/.op-tmp= and will be served locally by [[https://github.com/skeeto/emacs-web-server][simple-httpd]]. The =aog/site-preview-directory= variable is
  customizable. Be sure to read the documentation for =aog/do-publication-and-preview-site= for even more options.

* Helpful features

  1. Do you find that initializing a repository manually is too much trouble?
     The =aog/new-repository= command can simplify this.

  2. Want to make starting a post with essential properties much
     easier? Use the =aog/new-post= command.

  3. Already in a post buffer and just want to insert essential
     properties? Use the =aog/insert-options-template= command.

  4. Is the default slogan too ugly? Configure a better one (will be shown on each page):

     : (setq aog/site-main-title "your main slogan")
     : (setq aog/site-sub-title "your sub slogan")

  5. Is the branch you store org source files on not named "source"? It's
     easy to override the default branch names:

     : (setq aog/repository-org-branch "your source branch name") ;; default is "source"
     : (setq aog/repository-html-branch "your html branch name")  ;; default is "master"

  6. Have a github account? It's cool:

     : (setq aog/personal-github-link "your github link")

  7. Want to do site traffic analytics with Google Analytics?

     : (setq aog/personal-google-analytics-id "your google analytics id")

  8. Do you want to disable commenting for posts under certain
     categories? Yeah, it can be achieved:

     : (setq aog/category-config-alist
  ...
  10. Want to add an avatar to the page?
     ...
     : (setq aog/personal-avatar "URL to an image")

  11. Do not want to publish some categories?

     : (setq aog/category-ignore-list '("category1" "category2"))
